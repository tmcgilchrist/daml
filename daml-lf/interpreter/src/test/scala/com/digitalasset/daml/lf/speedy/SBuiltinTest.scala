...
class SBuiltinTest extends FreeSpec with Matchers with TableDrivenPropertyChecks {
  ...
  private def thenPowerOf(i: Int, scale: Int = 10) =
    if (i == 0)
      "0." + "0" * scale
    else if (i > 0)
      "1" + "0" * i + "." + "0" * scale
  ...
  "Decimal operations" - {
    ...
    "MUL_DECIMAL" - {
      "throws exception in case of overflow" in {
        ...
        eval(e"MUL_DECIMAL ${thenPowerOf(13)} ${thenPowerOf(14)}") shouldBe Right(
          SDecimal(decimal(1E27)))
        eval(e"MUL_DECIMAL ${thenPowerOf(14)} ${thenPowerOf(14)}") shouldBe 'left
    ...
    "DIV_DECIMAL" - {
      "throws exception in case of overflow" in {
        ...
        eval(e"DIV_DECIMAL $bigBigDecimal ${thenPowerOf(-10)}") shouldBe 'left
        eval(e"DIV_DECIMAL ${thenPowerOf(17)} ${thenPowerOf(-10)}") shouldBe Right(
          SDecimal(decimal(1E27)))
        eval(e"DIV_DECIMAL ${thenPowerOf(18)} ${thenPowerOf(-10)}") shouldBe Left(
          DamlEArithmeticError(
      ...
      "throws exception when divided by 0" in {
        eval(e"DIV_DECIMAL 1.0 ${thenPowerOf(-10)}") shouldBe Right(SDecimal(decimal(1E10)))
